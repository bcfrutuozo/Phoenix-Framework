...
public:
    ...
    */
    ...
    template<typename T, enable_if_t<is_promotion_wrapper<T>::value, bool> = true>
    constexpr CodePoint& operator=(T const& wrapper) noexcept requires(is_promotion_wrapper<T>::value) { Value = wrapper; return *this; };
     ...
     */
    template<typename T, enable_if_t<is_promotion_primitive<T>::value, bool> = true>
    friend inline constexpr CodePoint operator+(CodePoint const& lhs, T const& rhs) noexcept requires(is_promotion_primitive<T>::value) { return lhs.Value + rhs; }

    template<typename T, enable_if_t<is_promotion_wrapper<T>::value, bool> = true>
    friend inline constexpr CodePoint operator+(CodePoint const& lhs, T const& rhs) noexcept requires(is_promotion_wrapper<T>::value) { return lhs.Value + rhs; }

    template<typename T, enable_if_t<is_promotion_primitive<T>::value, bool> = true>
    friend inline constexpr CodePoint operator+(T const& lhs, CodePoint const& rhs) noexcept requires(is_promotion_primitive<T>::value) { return lhs + rhs.Value; }
    ...
    */
    template<typename T, enable_if_t<is_promotion_primitive<T>::value, bool> = true>
    friend inline constexpr UInt32 operator*(CodePoint const& lhs, T const& rhs) noexcept requires(is_promotion_primitive<T>::value) { return static_cast<uint32_t>(lhs.Value) * rhs; }

    template<typename T, enable_if_t<is_promotion_wrapper<T>::value, bool> = true>
    friend inline constexpr UInt32 operator*(CodePoint const& lhs, T const& rhs) noexcept requires(is_promotion_wrapper<T>::value) { return static_cast<uint32_t>(lhs.Value) * rhs; }

    template<typename T, enable_if_t<is_promotion_primitive<T>::value, bool> = true>
    friend inline constexpr UInt32 operator*(T const& lhs, CodePoint const& rhs) noexcept requires(is_promotion_primitive<T>::value) { return lhs * static_cast<uint32_t>(lhs.Value); }
    ...
    */
     ...
     */
    template<typename T, enable_if_t<is_promotion_primitive<T>::value&& is_integral<T>::value, bool> = true>
    friend inline constexpr UInt32 operator%(CodePoint const& lhs, T const& other) noexcept requires(is_promotion_primitive<T>::value&& is_integral<T>::value) {
        return static_cast<uint32_t>(lhs.Value) % other;
    }

    template<typename T, enable_if_t<is_promotion_wrapper<T>::value&& is_wrapper_integral<T>::value, bool> = true>
    friend inline constexpr UInt32 operator%(CodePoint const& lhs, T const& other) noexcept requires(is_promotion_wrapper<T>::value&& is_wrapper_integral<T>::value) {
        return static_cast<uint32_t>(lhs.Value) % other;
    }

    template<typename T, enable_if_t<is_promotion_primitive<T>::value&& is_integral<T>::value, bool> = true>
    friend inline constexpr UInt32 operator%(T const& lhs, CodePoint const& rhs) noexcept requires(is_promotion_primitive<T>::value&& is_integral<T>::value)
    {
        return lhs % static_cast<uint32_t>(lhs.Value);
    }
    ...
    template<typename T, enable_if_t<is_promotion_wrapper<T>::value, bool> = true>
    friend inline constexpr Boolean operator==(CodePoint const& lhs, T const& rhs) noexcept requires(is_promotion_wrapper<T>::value) { return lhs.Value == rhs; }
